/**
 * TX school board members for top-10 TX counties. Same pattern as countyBoards.js --
...
async function scrape() {
  ...
  for (const loc of localities) {
    ...
    try {
      ...
      for (const mem of members) {
        ...
        items.push({
          id: makeId('TX', 'school-board', (normLoc || '').toLowerCase(), cleaned.toLowerCase()),
          data: {
  ...
  console.log(`TX school boards total: ${items.length}`);
  return items;
}
