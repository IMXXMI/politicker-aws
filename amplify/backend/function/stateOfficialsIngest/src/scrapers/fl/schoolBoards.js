/**
 * FL school board members for top-10 FL counties. Same pattern as countyBoards.js —
...
async function scrape() {
  ...
  for (const loc of localities) {
    ...
    try {
      ...
      for (const mem of members) {
        ...
        items.push({
          id: makeId('FL', 'school-board', (normLoc || '').toLowerCase(), cleaned.toLowerCase()),
          data: {
  ...
  console.log(`FL school boards total: ${items.length}`);
  return items;
}
