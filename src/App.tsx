...
import { getFirestore, collection, addDoc, onSnapshot, query, where, serverTimestamp, Timestamp } from 'firebase/firestore';
...
const RepModal: React.FC<{
  ...
  repPolls: any;
  onClose: () => void;
}> = ({ selectedRep, repDetails, repPolls, onClose }) => {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal">
        ...
        <p>
          <strong>Party:</strong> {selectedRep.party} |{' '}
          <strong>Level:</strong> {selectedRep.level} |{' '}
          <strong>Score:</strong> {selectedRep.score}%
        </p>
        <RepPollBar rep={selectedRep} repPolls={repPolls} />

        <div className="rep-bio"><h3>Bio</h3><p>{repDetails.bio || 'No bio available'}</p></div>
        <div className="rep-votes"><h3>Voting History</h3><ul>{repDetails.votes.map((v, i) => <li key={i}>{v}</li>)}</ul></div>
        <div className="rep-bills"><h3>Supported Bills</h3><ul>{repDetails.bills.map((b, i) => <li key={i}>{b}</li>)}</ul></div>
        <div className="rep-earmarks"><h3>Earmarks</h3><ul>{(repDetails.earmarks || []).map((e, i) => <li key={i}>{e}</li>)}</ul></div>
        <div className="rep-comments"><h3>Recent X Comments</h3><ul>{repDetails.comments.map((c, i) => <li key={i}>{c}</li>)}</ul></div>
      </div>
    </div>
  );
};
...
function App() {
  ...
  const fetchRepDetails = async (rep: Rep) => {
    ...
    try {
      const bio = await fetchBioFromBallotpedia(rep.name);
      setRepDetails({
        bio: bio,
  ...
  useEffect(() => {
    ...
    if (saved) setCustomPollVotes(JSON.parse(saved));
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const polls = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() as Omit<Poll, 'id'> }));
      if (polls.length === 0) {
        setCurrentPoll(null);
      ...
      setCurrentPoll(nationwide || polls[0]);
      setPollLoading(false);
    });
    return () => unsubscribe();
  }, [user, zip]);
  ...
  const verifyVoter = async (fullAddress: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert('Address not recognized — try a known address');
        return;
      }
    ...
    try {
      if (!earmark && !comment && !repName) {
        ...
        setRepPolls(prev => {
          ...
          let pollKeyLocal: 'approve' | 'disapprove';
          if (choice === 'yea' || choice === 'approve') pollKeyLocal = 'approve';
          else if (choice === 'nay' || choice === 'disapprove') pollKeyLocal = 'disapprove';
          else {
            alert('Invalid vote choice for rep poll.');
            return prev;
          }
          const updatedTierEntry = { ...tierEntry, [pollKeyLocal]: (tierEntry[pollKeyLocal] || 0) + 1 };
  ...
  const getElectionId = async () => {
    ...
    try {
      ...
      return data.elections?.[0]?.id || null;
    } catch (err) {
      console.error('getElectionId error:', err);
      return null;
    }
  };

  const loadUserPolls = async (uid: string) => {
    console.log('loadUserPolls called for uid:', uid);
  };
              ...
              {reps
                .filter((rep) => {
                  if (activeTab === 'federal') {
                    return rep.level.includes('federal') || 
                           rep.level === 'President' || 
                           rep.level === 'Vice President' || 
                           rep.level === 'Supreme Court' || 
                           rep.level === 'Cabinet';
                  }
                  if (activeTab === 'state') {
